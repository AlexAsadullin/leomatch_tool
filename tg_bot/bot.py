...
- `TG_BOT_ADMIN_IDS` в .env — список user_id через запятую, кому разрешено
  управлять. Чужие сообщения игнорируются.
"""
...
from app.config import API_HASH, API_ID, ROOT
...
class TgBot:
    ...
    async def start(self) -> None:
        ...
        log.info("Telegram bot started; admin_ids=%s", sorted(self.admin_ids))
    ...
    async def _on_message(self, event: events.NewMessage.Event) -> None:
        ...
        if uid not in self.admin_ids:
            # echo their id so they can configure themselves as admin
            try:
                await event.respond(
                    f"Доступ запрещён. Твой user_id: <code>{uid}</code>. "
                    f"Добавь его в TG_BOT_ADMIN_IDS в .env и перезапусти.",
                    parse_mode="html",
                )
            except Exception:
                pass
...
def init(token: str, admin_ids: list[int]) -> TgBot:
    global _bot
    _bot = TgBot(token, admin_ids)
    return _bot
